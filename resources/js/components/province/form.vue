    ...
    export default {
        ...
        methods: {
            init(){
                ...
                window.axios.get(`/admin/portal/province/${this.id}/view`).then((response) => {
                    this.form.extract(response.data.body.model);
                    this.form.loading = false;
                });
            },
            create(){
                this.form.submit(this.edit ? `/admin/portal/province/${this.id}/update`  : '/admin/portal/province/create').then((response) => {
                    window.alerts.success(response).then((response) => {
                        if (!this.edit)
                        this.$router.push(`/admin/province/${response.data.body.model.id}/view`);
                    });
                }).catch((error) => {
                }).finally(() => {
                });
        ...
        <div class="col-lg-12">
            <div :class="['dimmer' , form.loading ? 'active' : '']">
                ...
                <div class="dimmer-content">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="mb-3"> {{ this.edit ? 'Edit' : 'Create' }}  Province</h4>
                            <div class="form-horizontal">
                                ...
                                <div class="form-group mb-0 justify-content-end row">
                                    <div class="col-lg-9">
                                        <button type="submit" @click.prevent="create" :class="['btn btn-primary' , form.loading ? 'btn-loading' : '']">{{ this.edit ? 'Edit' : 'Create' }} Province</button>
                                    </div>
                                </div>
                            </div>
                        </div>
